...
class TicTacToe:
    ...
    def __make_move(self, move):
        ...
        :param Move move: Move with which to update the board
        """
        self.__check_if_move_is_legal(move)
        self.board[move.row][move.column] = self.current_move_marker

    def __check_if_move_is_legal(self, move):
    ...
    def __change_player(self):
        if self.current_move_marker == 'X':
            self.current_move_marker = 'O'
        else:
            self.current_move_marker = 'X'
